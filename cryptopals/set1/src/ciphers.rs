use thiserror::Error;
use crate::ciphers::MeanNormalizedHammingDistanceScoreError::{KeySizeAndSamplesTooLargeForCipherLength, TooFewSamples};
...
pub fn get_best_key_sizes_and_decryptions_of_repeating_key_xor(
    ...
    key_sizes_and_scores
        ...
        .map(|(key_size, _)| {
            let decrypted_bytes =
                decrypt_repeating_key_xor_given_key_size(&encrypted_bytes, *key_size);
...
pub fn decrypt_repeating_key_xor_given_key_size(
    encrypted_bytes: &[u8],
    key_size: usize,
) -> Vec<u8> {
...
pub fn get_mean_normalized_hamming_distance_score_for_key_size(
    ...
    if cipher_bytes.len() < num_samples * key_size {
        return Err(KeySizeAndSamplesTooLargeForCipherLength {
