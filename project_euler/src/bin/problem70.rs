...
fn solve() -> N {
    ...
    // 6. Once fully factorized, check if n/phi(n) is less than the min ratio found so far.
    ...
    // 7. If it is, or if no minimum ratio has been found, check if phi(n) is a permutation of n.
    // 8. If it is, then update best_n and best_phi.
    ...
    for sqrt_n in 2..=sqrt_max_n {
        ...
        'next_n: for _ in 0..(2 * sqrt_n + 1) {
            ...
            try_reduce_by_prime_factor(&mut reduced_n, &mut phi, 2);
            try_reduce_by_prime_factor(&mut reduced_n, &mut phi, 3);
            for mid_p in (6..=sqrt_n).step_by(6) {
                if try_reduce_by_prime_factor(&mut reduced_n, &mut phi, mid_p - 1) {
                    if reduced_n == 1 {
                        break;
                    }
                    if phi < pow_of_10 {
