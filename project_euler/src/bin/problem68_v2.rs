...
fn solve() -> u64 {
    ...
    let search_node: SearchNode = SearchNode {
        curr_solution: 0, // empty curr_solution
        magic_number: 0,  // fake magic_number, since not yet chosen
        used_numbers: 0,  // no numbers have been used yet

        // We want to filter to only the first D digits in the best solution, when comparing
        // to the current solution, which has D digits. So keep track of the power of 10
        // to divide the best solution by, to make it comparable with the current solution...
        pow_of_10: 10_u64.pow(16),
    };
...
fn apply_instructions(
    ...
    if instructions.is_empty() {
        ...
        if let Some(best_value) = best_solution {
            if search_node.curr_solution > *best_value {
                *best_solution = Some(search_node.curr_solution);
            }
        } else {
            *best_solution = Some(search_node.curr_solution);
        }
    ...
    match instruction {
        ...
        Instruction::AppendDigitsForPreviousPositionToSolution { position } => {
            ...
            append_digits_of_number_to_solution(number, &mut search_node);
            if incapable_of_being_higher_than_best_solution(&mut search_node, best_solution) {
                return Outcome::Other;
            }
            apply_instructions(curr_layout, search_node, best_solution, &instructions[1..])
        }
    }
}
...
fn try_possible_magic_numbers_in_descending_order(
    ...
    let n = curr_layout[0].unwrap(); // The first number chosen (i.e., in position 0) 
    ...
    for magic_number in (min_magic_number..=max_magic_number).rev() {
        search_node.magic_number = magic_number;
        apply_instructions(curr_layout, search_node, best_solution, &instructions[1..]);
    }
    Outcome::Other
}
...
fn attempt_number_that_makes_line_sum_add_up_to_magic_number(
    ...
    if line_sum >= search_node.magic_number {
        return Outcome::Other;
    }
    let number = search_node.magic_number - line_sum;
    if number > 10 {
...
fn attempt_number_in_position(
    ...
    if is_number_already_used(number, search_node.used_numbers) {
        return Outcome::Other;
    }
    append_digits_of_number_to_solution(number, &mut search_node);
    if incapable_of_being_higher_than_best_solution(&mut search_node, best_solution) {
    ...
    mark_number_as_used(number, &mut search_node.used_numbers);
...
fn incapable_of_being_higher_than_best_solution(
    ...
    if let Some(best_solution) = best_solution {
        ...
        let best_solution_truncated = *best_solution / search_node.pow_of_10;
        if search_node.curr_solution < best_solution_truncated {
...
fn append_digits_of_number_to_solution(number: u8, search_node: &mut SearchNode) {
    if number == 10 {
        search_node.pow_of_10 /= 100;
        search_node.curr_solution *= 100;
    } else {
        search_node.pow_of_10 /= 10;
        search_node.curr_solution *= 10;
    }
    search_node.curr_solution += number as u64;
}
