...
fn find_min_sum_of_cyclic_figurate_numbers(
    ...
    if level == 5 {
        ...
        graph_nodes[(encoded_digits % 100) as usize]
            .iter()
            .filter_map(|edge| {
                let fig_num_type_index = edge.figurate_number_type as usize;
                if fig_types_used[fig_num_type_index] {
                    None
                } else {
                    let mut new_fig_types_used = fig_types_used;
                    new_fig_types_used[fig_num_type_index] = true;
