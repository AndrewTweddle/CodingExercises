...
fn solve() -> u64 {
    ...
    let mut cyclic_figurate_numbers: Vec<[u16; 6]> = Vec::new();
        ...
        for permutation in permutations.iter() {
            ...
            for &index in permutation {
                encoded_digits = encoded_digits
                    .iter()
                    .flat_map(|&digits| {
                        lookups[index as usize]
                            .get(&((digits % 100) as u8))
                            .unwrap_or(&vec![])
                            .iter()
...
fn get_digit_lookups_by_figurate_number_type() -> Vec<HashMap<u8, Vec<u8>>> {
    let lookups: Vec<HashMap<u8, Vec<u8>>> = (0..5)
        .map(|fig| {
            let mut first2_to_last2: HashMap<u8, Vec<u8>> = HashMap::new();
            for n in 1_u64.. {
                ...
                let hundreds = (fig_num / 100) as u8;
                first2_to_last2
                    .entry(hundreds)
                    .or_default()
                    .push(tens_and_ones);
